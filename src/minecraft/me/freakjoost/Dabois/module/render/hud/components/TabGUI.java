package me.freakjoost.Dabois.module.render.hud.components;

import com.mojang.realmsclient.gui.ChatFormatting;
import me.freakjoost.Dabois.Dabois;
import me.freakjoost.Dabois.event.EventPriority;
import me.freakjoost.Dabois.event.EventTarget;
import me.freakjoost.Dabois.event.events.EventKeyboard;
import me.freakjoost.Dabois.event.events.EventRender2D;
...
public class TabGUI {
    ...
    public void onRedner(EventRender2D e) {
        ...
        if (screen == 1 || screen == 2) {
            ...
            for (Module m : getModsForCurrentCategory()) {
                ...
                fr.drawStringWithShadow(m.getName() + (Dabois.INSTANCE.SETTING_MANAGER.getSettingsForModule(m) != null ? ">" : ""), startModsX + 2 + (this.getCurrentModule().equals(m) ? 2 : 0),
    ...
    private void renderTopString(int x, int y) {
        fr.drawStringWithShadow(ChatFormatting.WHITE + Dabois.INSTANCE.NAME + ChatFormatting.RESET + " v"
                + Dabois.INSTANCE.VERSION, x, y, new Color(67, 0, 99).getRGB());
    }
    ...
    private ArrayList<Setting> getSettingForCurrentMod() {
        return Dabois.INSTANCE.SETTING_MANAGER.getSettingsForModule(getCurrentModule());
    }
    ...
    private ArrayList<Module> getModsForCurrentCategory() {
        ...
        for (Module m : Dabois.INSTANCE.MODULE_MANAGER.getModules()) {
